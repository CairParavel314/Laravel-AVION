...
<div class="border border-gray-200 rounded-lg p-6 hover:shadow-md transition">
    <div class="flex items-center justify-between mb-4">
        <div class="flex items-center">
            ...
            <a href="{{ route('granjas.show', $granja) }}" class="ml-3 text-lg font-semibold text-gray-900 hover:text-blue-600 transition">
                {{ $granja->nombre }}
            </a>
        </div>
        ...
        <p class="text-sm text-gray-500">{{ Str::limit($granja->descripcion, 100) }}</p>
                        ...
                        <div class="mt-4 flex space-x-2">
                            <a href="{{ route('granjas.show', $granja) }}"
                                class="flex-1 bg-blue-50 text-blue-700 py-2 px-3 rounded text-sm hover:bg-blue-100 transition text-center">
                                <i class="fas fa-eye mr-1"></i>Ver
                            </a>
                            <a href="{{ route('lotes.create') }}?granja_id={{ $granja->id }}"
                                class="flex-1 bg-green-50 text-green-700 py-2 px-3 rounded text-sm hover:bg-green-100 transition text-center"
                                title="Crear lote en esta granja">
                                <i class="fas fa-plus mr-1"></i>Lote
                            </a>
                            <a href="{{ route('granjas.edit', $granja) }}"
                                class="flex-1 bg-yellow-50 text-yellow-700 py-2 px-3 rounded text-sm hover:bg-yellow-100 transition text-center">
                            ...
                            <form action="{{ route('granjas.destroy', $granja) }}" method="POST" class="flex-1">
                                ...
                                @method('DELETE')
                                <button type="submit"
                                    class="w-full bg-red-50 text-red-700 py-2 px-3 rounded text-sm hover:bg-red-100 transition"
