    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="bg-white rounded-lg shadow">
            ...
            <div class="p-6">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @foreach($ultimasPruebas as $prueba)
                                                <tr class="hover:bg-gray-50 transition">
                                                    <!-- Columna Lote - Clickable -->
                                                    <td class="px-4 py-3 text-sm font-medium">
                                                        <a href="{{ route('lotes.show', $prueba->lote) }}"
                                                            class="text-blue-600 hover:text-blue-900 transition block"
                                                            title="Ver detalles del lote">
                                                            {{ $prueba->lote->codigo_lote }}
                                                        </a>
                                                        <a href="{{ route('granjas.show', $prueba->lote->granja) }}"
                                                            class="text-xs text-gray-500 hover:text-gray-700 transition block mt-1"
                                                            title="Ver granja">
                                                            {{ $prueba->lote->granja->nombre }}
                                                        </a>
                                                    </td>

                                                    <!-- Columna Prueba - Clickable -->
                                                    <td class="px-4 py-3 text-sm">
                                                        <a href="{{ route('pruebas.show', $prueba) }}"
                                                            class="text-purple-600 hover:text-purple-900 font-medium transition block"
                                                            title="Ver detalles de la prueba">
                                                            {{ $prueba->parametro }}
                                                        </a>
                                                        <span class="text-xs text-gray-500 block mt-1">
                                                            {{ ucfirst($prueba->tipo_prueba) }} • {{ $prueba->valor }}
                ...
                <!-- Enlace para ver todas las pruebas -->
                <div class="mt-4 text-center">
                    <a href="{{ route('pruebas.index') }}" class="text-blue-600 hover:text-blue-900 text-sm font-medium">
                                ...
                                <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition">
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-600">
                                            <a href="{{ route('granjas.show', $lote->granja) }}"
                ...
                @if($lotesRecientes->isEmpty())
                    <div class="text-center py-4">
                        ...
                        <a href="{{ route('lotes.create') }}"
                            class="mt-2 inline-block bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition">
                            Crear primer lote
                        </a>
                    </div>
                @else
